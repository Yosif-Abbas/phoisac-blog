...
export default function PostItem({ post }: { post: Post }) {
  ...
  // Clean up Editor.js HTML tags if they exist in the preview text
  const rawText =
    post.content.blocks.find((block) => block.type === "paragraph")?.data
      ?.text || "";

  const description = rawText.replace(/<[^>]*>?/gm, ""); // Strips HTML tags

  return (
    <li className="list-none group">
      <Link
        href={`/blog/${post.slug}`}
        className="relative block p-6 rounded-2xl border border-transparent hover:border-[#E5E7EB] dark:hover:border-[#1F2937] hover:bg-container/50 transition-all duration-300 shadow-sm hover:shadow-md"
      >
